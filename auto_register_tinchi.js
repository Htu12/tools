(function () {
  ...
  function extractRowInfo(row, idx0) {
    const clsEl = row.querySelector(
      `[id^='${C.ID_PREFIX_COURSE_CLASS}${idx0}']`,
    );
    ...
    return {
      ...
      radio:
        ...
        row.querySelector('input[id*="rdiSelect"]'),
    };
  }
  ...
  async function bootstrap() {
    ...
    while (true) {
      ...
      if (performance.now() - t0 > CONFIG.MAX_RUNTIME_MS) {
        console.warn(
          "[AutoRegister] Hết phiên làm việc theo chính sách MAX_RUNTIME_MS. Kết thúc.",
        );
        return;
      }
      ...
      if (!row) {
        console.warn(
          `[AutoRegister] Không tìm thấy hàng lớp index=${idx1}. Sẽ ViewCourse lại.`,
        );
        await safeClick(btnView);
        await sleep(CONFIG.VIEW_RETRY_BACKOFF_MS);
        await sleep(jitter(CONFIG.POLL_MS_MIN, CONFIG.POLL_MS_MAX));
      ...
      console.log(
        `[AutoRegister] Lớp ${info.courseClass} — Đã đăng ký ${info.registered}/${info.limit}`,
      );
      ...
      if (info.registered < info.limit && info.radio) {
        ...
        console.log(
          `[AutoRegister] ${clicked ? "ĐÃ BẤM" : "KHÔNG THỂ BẤM"} nút Đăng ký`,
        );
